...
use App\Filament\Resources\NdtInstructionResource\Pages;
use App\Filament\Resources\NdtInstructionResource\RelationManagers;
use App\Models\NdtInstruction;
use Filament\Forms;
...
class NdtInstructionResource extends Resource
{
    protected static ?string $model = NdtInstruction::class;
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                ...
                ->default(fn () => NdtInstruction::generateNextNumber())
                  ->readOnly()
                    ->required(),
 ...
 ->getUploadedFileNameForStorageUsing(function (TemporaryUploadedFile $file, callable $get) {
    ...
    Forms\Components\DatePicker::make('create_date')
            ...
            BelongsToSelect::make('approved_by')
                ...
                Forms\Components\Select::make('status')
                    ->label('Status')
    ->options(NdtInstruction::getStatusOptions())
    ->default(NdtInstruction::STATUS_PENDING_REVIEW)
    ->required(),
    ...
    ->label('Status')
    ->formatStateUsing(fn ($state) => NdtInstruction::getStatusOptions()[$state] ?? 'Unknown')
    ->badge()
    ->color(fn ($state) => match ($state) {
        NdtInstruction::STATUS_APPROVED => 'success',
        NdtInstruction::STATUS_PENDING_REVIEW => 'warning',
        NdtInstruction::STATUS_PENDING_APPROVAL => 'info',
        NdtInstruction::STATUS_WITHDRAWN => 'danger',
        default => 'gray',
    }),
...
Tables\Actions\Action::make('downloadFile')
    ...
    {
        return [
            'index' => Pages\ListNdtInstructions::route('/'),
            'create' => Pages\CreateNdtInstruction::route('/create'),
            'edit' => Pages\EditNdtInstruction::route('/{record}/edit'),
        ];
    }
}
