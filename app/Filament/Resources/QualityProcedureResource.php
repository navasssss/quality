...
use App\Filament\Resources\QualityProcedureResource\Pages;
use App\Filament\Resources\QualityProcedureResource\RelationManagers;
...
{
    protected static ?string $model = QualityProcedure::class;
 ...
 public static function form(Form $form): Form
    {
        return $form
            ->schema([
                ...
                ->default(fn () => QualityProcedure::generateNextNumber())
                  ->readOnly()
                    ->required(),
    ...
    ->disk('public_uploads')
    ->directory('quality_procedure')
    ->moveFiles()
    ->label('Upload Quality Procedure')
    ->acceptedFileTypes([
        'application/pdf',
        'application/msword',
 ...
 ->getUploadedFileNameForStorageUsing(function (TemporaryUploadedFile $file, callable $get) {
    ...
    Forms\Components\DatePicker::make('create_date')
            ...
            BelongsToSelect::make('approved_by')
                ...
                Forms\Components\Select::make('status')
                    ->label('Status')
    ->options(QualityProcedure::getStatusOptions())
    ->default(QualityProcedure::STATUS_PENDING_REVIEW)
    ->required(),
    ...
    ->label('Status')
    ->formatStateUsing(fn ($state) => QualityProcedure::getStatusOptions()[$state] ?? 'Unknown')
    ->badge()
    ->color(fn ($state) => match ($state) {
        QualityProcedure::STATUS_APPROVED => 'success',
        QualityProcedure::STATUS_PENDING_REVIEW => 'warning',
        QualityProcedure::STATUS_PENDING_APPROVAL => 'info',
        QualityProcedure::STATUS_WITHDRAWN => 'danger',
        default => 'gray',
    }),
...
Tables\Actions\Action::make('downloadFile')
    ...
    {
        return [
            'index' => Pages\ListQualityProcedures::route('/'),
            'create' => Pages\CreateQualityProcedure::route('/create'),
            'edit' => Pages\EditQualityProcedure::route('/{record}/edit'),
        ];
    }
}
